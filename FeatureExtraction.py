...
def infer_time_series_params(
    ...
    try:
        ...
        # print(
        # f"Seasonal period detection failed: {e}. Using default for
        # {frequency}.")
        seasonal_period = FREQ_CONFIG[frequency]['seasonal_period']
    ...
    try:
        ...
        # print(
        # f"Rolling window inference failed: {e}. Using default for
        # {frequency}.")
...
def process_single_series(ts_df, freq, dataset_name):
    ...
    try:
        ...
        # print(
        # f"Error in {ts_df.columns[0] if not ts_df.empty else 'unknown
        # series'}: {e}")
        return False
    finally:
        gc.collect()
